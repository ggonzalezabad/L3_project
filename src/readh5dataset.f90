...
MODULE ReadH5Dataset
  ...
  ! -----------------------------------------------------------
  ! January 2013, Modified version to compile with gfortran gga
  ! Created H5ReadReal4D,
  ! Created H5Read3DRealdims to get dimensions (lines, xtrack, wav)
  ! -----------------------------------------------------------
...
! *******************************************************************
  ...
  INTERFACE H5ReadDataset
     MODULE PROCEDURE  &
      ...
      & H5ReadInteger4D, H5ReadReal0D,    H5ReadReal1D,    H5ReadReal2D,    &
      & H5ReadReal3D,    H5ReadReal4D,    H5Read3DRealdims,                 &
      & H5ReadDouble0D,  H5ReadDouble1D,  H5ReadDouble2D,  H5ReadDouble3D,  &
      & H5ReadDouble4D,  H5ReadString0D,  H5ReadString1D,                   &
  ...
  SUBROUTINE DebugMessage (DebugStr,DebugValue)
    ...
    IF (DebugMsg) THEN
       IF (PRESENT(DebugValue)) THEN
          write(6,'(a,1x,i2)')DebugStr,DebugValue
       ELSE
          write(6,'(a)')DebugStr
       ENDIF
    ENDIF
...
! *******************************************************************
  ...
  SUBROUTINE H5ReadReal4D        &
    ...
    CALL DebugMessage(" === in H5ReadReal4D ===")
  ...
  END SUBROUTINE H5ReadReal4D

 ! *********************************************************************

  SUBROUTINE H5Read3DRealdims    &
       (                         &
       H5filename,               & !Coming in
       datasetname,              & !Coming in
       nLines,nXtrack,nWavel     &  !Going out
       )
    !
    !<<<<<<<<<<<<<<<<<<<<<<< Implicit statement >>>>>>>>>>>>>>>>>>>>>
    IMPLICIT NONE
    ...
    !Going out:
    INTEGER*4, INTENT(out)                               :: nLines,nXtrack,nWavel
...
!    INTEGER                                              :: ndims
    ...
    CALL DebugMessage(" === in H5Read3DRealdims ===")
    ...
    IF (ErrorFlag.lt.0) THEN
       ...
       return
    ENDIF
    nLines  = INT(datadims(3), KIND=4)
    nXtrack = INT(datadims(2), KIND=4)
    nWavel  = INT(datadims(1), KIND=4)
    ! Close all that is open
...
! *******************************************************************
  ...
  SUBROUTINE H5ReadDouble4D      &
    ...
    CALL DebugMessage(" === in H5ReadDouble4D ===")
...
!    INTEGER(hsize_t), DIMENSION(1)                       :: maxdatadims
    ...
    IF (INT(strlen, KIND=4) .gt. LEN(dataset)) THEN
...
! *******************************************************************
  ...
  SUBROUTINE H5ReadString1D      &
    ...
    IF (strlen.gt.INT(LEN(dataset),KIND=8)) THEN
       ErrorMessage=" *** Error: string of insufficient length defined"
       ErrorFlag=-1
       return
    ENDIF
    ...
    do indx=1, INT(dims(1), KIND=4)
       iend=INT(ibeg, KIND=4)+INT(strlen, KIND=4)-1 ! end index along string at index idts
       if (iend.le.deflen) then  ! still fully in current idts
          ...
          substr = H5dataset(idts)(INT(ibeg, KIND=8):INT(deflen, KIND=8)) &
                // H5dataset(idts+1)(INT(1, KIND=8):strlen-(INT(deflen, KIND=8)-INT(ibeg, KIND=8)+INT(1, KIND=8)))
          idts=idts+1
          ibeg=INT(strlen, KIND=4) - (INT(deflen, KIND=4) - ibeg + &
               INT(1, KIND=4))     + INT(1, KIND=4)
       endif
